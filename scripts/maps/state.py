"""Download Census TIGER state boundaries and write per-state state.geojson."""
import json
import zipfile
...
def build_state_map_for_state(state: str, fips: str) -> str:
    """Filter the national state shapefile to this state and write state.geojson."""
    output_path = PROJECT_ROOT / "data" / state / ".maps" / "state.geojson"
